...
__all__ = ("PPAPEMA", "PoolingEMA", "KDEHyperGraphFusion", "GraphHyperGraphFusion", "CLAGRGCUFuse")
...
class DescriptorPool(nn.Module):
    """Shared descriptor pooling used for reviewer-requested EMA pooling ablations."""

    def __init__(self, mode: str = "ppap", topk_ratio: float = 0.05, topk_min: int = 8, topk_max: int = 64):
        super().__init__()
        self.mode = str(mode).lower().replace("-", "_").replace("+", "_")
        self.ppap = PPAP(topk_ratio=topk_ratio, topk_min=topk_min, topk_max=topk_max)
        valid = {"gap", "gmp", "gap_gmp", "top1", "ppap"}
        if self.mode not in valid:
            raise ValueError(f"unknown pooling mode '{mode}', expected one of {sorted(valid)}")

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.mode == "gap":
            return x.mean(dim=(2, 3), keepdim=True)
        if self.mode in {"gmp", "top1"}:
            return x.amax(dim=(2, 3), keepdim=True)
        if self.mode == "gap_gmp":
...
class PoolingEMA(PPAPEMA):
    """PPAP-EMA structure with a swappable descriptor pooling statistic."""
    ...
    def __init__(
        ...
        self.pooling_mode = str(mode).lower()
        self.ppap = DescriptorPool(
...
class GraphHyperGraphConv(nn.Module):
    """Hypergraph propagation with alternative incidence construction strategies."""
    ...
    def __init__(
        ...
        super().__init__()
        self.c = int(channels)
        self.strategy = str(strategy).lower().replace("-", "_")
        self.k = int(k)
        self.threshold = float(threshold)
        self.random_prob = float(random_prob)
        valid = {"none", "random", "knn", "fixed", "epsilon", "cosine", "kde"}
        if self.strategy not in valid:
            raise ValueError(f"unknown graph strategy '{strategy}', expected one of {sorted(valid)}")
    ...
    def _build_incidence(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        b, m, _ = x.shape
        if m == 1:
            H = torch.ones((b, 1, 1), device=x.device, dtype=x.dtype)
            dist = torch.zeros((b, 1, 1), device=x.device, dtype=x.dtype)
        ...
        eye_bool = torch.eye(m, device=x.device, dtype=torch.bool).unsqueeze(0)
        eye = torch.eye(m, device=x.device, dtype=x.dtype).unsqueeze(0)
        valid = ~eye_bool
        dist_valid = dist.masked_select(valid).reshape(b, -1)
        rho = torch.zeros((b, m), device=x.device, dtype=x.dtype)

        if self.strategy == "none":
            H = eye.expand(b, -1, -1).clone()

        elif self.strategy == "kde":
            h = dist_valid.mean(dim=1, keepdim=True).clamp_min(1e-6)
            h2 = (h ** 2).view(b, 1, 1)
            rho = torch.exp(-(dist ** 2) / (2.0 * h2)).sum(dim=-1)
            rho_member = rho.unsqueeze(2)
            rho_edge = rho.unsqueeze(1)
            H = (rho_member > rho_edge).to(x.dtype)

        elif self.strategy == "random":
            prob = min(max(self.random_prob, 0.0), 1.0)
            H = (torch.rand((b, m, m), device=x.device) < prob).to(x.dtype)

        elif self.strategy == "knn":
            k = max(1, min(self.k, m - 1))
            masked = dist.masked_fill(eye_bool, float("inf"))
            idx = masked.topk(k=k, dim=2, largest=False).indices
            H = torch.zeros((b, m, m), device=x.device, dtype=x.dtype)
            batch_idx = torch.arange(b, device=x.device)[:, None, None]
            edge_idx = torch.arange(m, device=x.device)[None, :, None]
            H[batch_idx, idx, edge_idx] = 1.0

        elif self.strategy in {"fixed", "epsilon"}:
            eps = dist_valid.mean(dim=1, keepdim=True).view(b, 1, 1).clamp_min(1e-6)
            eps = eps * max(self.threshold, 1e-6)
            H = (dist <= eps).to(x.dtype)

        else:  # cosine threshold
            sim = cosine_sim(x, x)
            H = (sim >= self.threshold).to(x.dtype)

        H = torch.clamp(H + eye, max=1.0)
        return H, dist, rho
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        De = H.sum(dim=1).clamp_min(1e-6)
        Dv = H.sum(dim=2).clamp_min(1e-6)
        De_inv = 1.0 / De
        Dv_inv_sqrt = 1.0 / torch.sqrt(Dv)
...
class GraphHyperGraphCompute(nn.Module):
    """Feature map -> prototypes -> alternative graph propagation -> back projection."""
    ...
    def __init__(
        ...
        self.hyper = GraphHyperGraphConv(
            channels=self.c,
            strategy=strategy,
            k=k,
            threshold=threshold,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        denom = w_assign.sum(dim=1).unsqueeze(-1).clamp_min(1e-6)
        nodes = torch.einsum("btm,btc->bmc", w_assign, tokens) / denom
        nodes_out = self.hyper(nodes)
        tokens_out = torch.einsum("btm,bmc->btc", w_assign, nodes_out)
        feat_out = tokens_out.reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()
        y = x + self.out_proj(feat_out)
...
class GraphHyperGraphFusion(nn.Module):
    """Multi-level fusion with swappable hypergraph construction."""
    ...
    def __init__(
        ...
        self.proj = nn.ModuleList([ConvBNAct(ci, branch_c, k=1, s=1) for ci in self.in_channels])
        self.fuse = ConvBNAct(branch_c * len(self.in_channels), self.out_c, k=1, s=1)
        self.hg = GraphHyperGraphCompute(
            channels=self.out_c,
            num_nodes=num_nodes,
            cluster_iters=3,
            tau=tau,
